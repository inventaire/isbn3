...
For quick browser debugging, you can temporarily add `debugger;` where you want execution to stop.

If you add `debugger;` in `isbn.js` or a file under `lib/`, rebuild with `npm run dev:update-dist` and then refresh the page so the updated `dist/isbn.js` bundle is loaded.

You can also use the browser developer tools to set breakpoints without editing the source.
